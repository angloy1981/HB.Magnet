<template>
  <div class="select-tree single-select-tree">
    ...
    <el-tree ref="tree"
             ...
             @node-click="eHandleNodeClick"
             :filter-node-method="eFilterNode">
...
export default {
  ...
  mounted() {
    ...
    window.setAppend = this.setAppend
    window.getCurrentNode = this.getCurrentNode
    window.setTree = this.setTree
  ...
  methods: {
    ...
    eHandleNodeClick(data, node) { // 点击节点
      // 提交节点点击事件
...
export function AppendTreeNode(value) {
  window.setAppend(value)
}
export function GetTreeCurrentNodeKey() {
  return window.getCurrentNode()
}
export function SetTree(treeDatas1) {
  window.setTree(treeDatas1)
}
</script>
